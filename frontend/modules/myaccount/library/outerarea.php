...
if ( ! class_exists( 'ClassFeild' ) )
{
	class ClassFeild
	{
		...
		static function show( $field, $saved )
		{
			...
			$begin = call_user_func( array( $field_class, 'starthtml' ), $meta, $field );

			// Apply filter to field begin HTML
			// 1st filter applies to all fields
			// 2nd filter applies to all fields with the same type
			// 3rd filter applies to current field only
			$begin = apply_filters( 'rwmb_starthtml', $begin, $field, $meta );
			$begin = apply_filters( "rwmb_{$type}_starthtml", $begin, $field, $meta );
			$begin = apply_filters( "rwmb_{$id}_starthtml", $begin, $field, $meta );
			...
			if ( $field['clone'] )
			{
				...
				foreach ( $meta as $index => $sub_meta )
				{
					...
					// Remove clone button
					$input_html .= call_user_func( array( $field_class, 'unsetclonebtn' ), $sub_field );
			...
			$end = call_user_func( array( $field_class, 'finishhtml' ), $meta, $field );

			// Apply filter to field end HTML
			// 1st filter applies to all fields
			// 2nd filter applies to all fields with the same type
			// 3rd filter applies to current field only
			$end = apply_filters( 'rwmb_finishhtml', $end, $field, $meta );
			$end = apply_filters( "rwmb_{$type}_finishhtml", $end, $field, $meta );
			$end = apply_filters( "rwmb_{$id}_finishhtml", $end, $field, $meta );
		 ...
		 * @return string
		 */
		static function starthtml( $meta, $field )
		{
			$field_label = '';
			if ( $field['name'] )
			{
		 ...
		 * @return string
		 */
		static function finishhtml( $meta, $field )
		{
			$button = $field['clone'] ? call_user_func( array( RW_Meta_Box::get_class_name( $field ), 'setclonebtn' ), $field ) : '';
		 ...
		 * @return string $html
		 */
		static function setclonebtn( $field )
		{
			$text = apply_filters( 'rwmb_setclonebtn_text', esc_html__( '+', 'service-finder' ), $field );
		 ...
		 * @return string $html
		 */
		static function unsetclonebtn( $field )
		{
			$text = apply_filters( 'rwmb_unsetclonebtn_text', esc_html__( '&#8211;', 'service-finder' ), $field );
