...
{	
	...
	{
		/* settings */
		$font = SERVICE_FINDER_BOOKING_FONTS_DIR.'/calibri.ttf';/*define font*/
	...
	public function phpcaptcha($textColor,$backgroundColor,$imgWidth,$imgHeight,$noiceLines=0,$noiceDots=0,$noiceColor='#162453',$where = '')
	{	
		/* Settings */
		$text=$this->random();
		$font = SERVICE_FINDER_BOOKING_FONTS_DIR.'/monofont.ttf';/* font */
		$textColor=$this->hexToRGB($textColor);	
		...
		if(isset($_SESSION)){
			$_SESSION['captcha_code_'.$where] = $text;/* set random text in session for captcha validation*/
		}
	}
